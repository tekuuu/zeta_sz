...
- the full matrix spectral theorem from Hermitian diagonalization;
...
```bash
lake build --wfail
lake build ZetaSimpleZeros.Verify --wfail
```

CI also invokes the independent `nanoda` checker with `sorryAx` forbidden.
